...
- [ ] `Exception` nodes are visually distinct. They carry the tribal knowledge
...
- [ ] Nodes with no evidence are visibly flagged. `validate` should prevent
...
- [ ] Readable at a laptop window size; wide content (diagram, long source
      excerpts) scrolls in its own container rather than the page scrolling
      sideways.
- [ ] Works against `.json`, `.db` and `.sqlite` graphs (it only talks to the
